...
import { DAY_TEMPLATES, effectiveCoverage, SLOTS } from '@/data/coverageTemplate'
...
const DAY_ORDER = [4, 5, 6, 0, 1, 2, 3]  // Thu-Wed work week

export function CoverageGridEditor({ coverageOverrides, onSetOverride, onReset }: Props) {
      ...
      {open && (
        <div className="border-t border-slate-200 px-4 py-3">
          <p className="mb-3 text-xs text-slate-500">
            Edit the number of dispatchers needed in each slot. Empty rows
            use the day's baseline.
          </p>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full border-separate border-spacing-0 text-[11px]">
              ...
              <tbody>
                {DAY_ORDER.map((dow) => {
                  const baseline = DAY_TEMPLATES[dow].requiredCoverage
                  const override = coverageOverrides[dow]
                  const effective = effectiveCoverage(dow, coverageOverrides)
                  return (
                    <tr key={dow} className="border-t border-slate-100">
                      ...
                      {baseline.map((base, si) => {
                        ...
                        const isOverridden = override !== undefined && override[si] !== base
                        return (
                          <td key={si} className="px-0.5 py-1">
                            <input
                              ...
                              title={`Effective ${eff} · baseline ${base}`}
                              className={clsx(
