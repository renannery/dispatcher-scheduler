...
interface IndividualDocProps {
  ...
  schedule: GeneratedSchedule
  /** Hide all hour totals (peak week, per-week, per-day) — used when the
   *  PDF is generated by a non-admin user who shouldn't see the hours. */
  hideHours?: boolean
}
          ...
          return (
            <View key={wl} wrap={false}>
              <View style={SC.wHead}>
                ...
                {!hideHours && <Text style={SC.wHrs}>{wh.toFixed(1)}h</Text>}
              </View>
              {days.map((di) => {
                const entry = ds.days.find((d) => d.date === di.date)
                if (!entry) return null
...
export async function exportIndividualPDF(
  ...
  if (!ds) return
  const blob = await pdf(<IndividualDoc ds={ds} schedule={schedule} hideHours={hideHours} />).toBlob()
...
export async function exportIndividualCompactPDF(
  ...
  const ds = schedule.dispatcherSchedules.find((d) => d.dispatcher.id === dispatcherId)
  if (!ds) return
  const blob = await pdf(<CompactDoc ds={ds} schedule={schedule} hideHours={hideHours} />).toBlob()
  const safe = ds.dispatcher.name.replace(/[^a-z0-9]/gi, '_').toLowerCase()
